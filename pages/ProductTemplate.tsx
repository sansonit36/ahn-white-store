...
import { Star, Check, ShieldCheck, Truck, ChevronDown, ChevronUp, Droplets, Sparkles, Sun, Fingerprint, Info, X, AlertOctagon, HeartHandshake, Target, Camera, Flame } from 'lucide-react';
...
export const ProductTemplate: React.FC = () => {
    ...
    // If ID matches a product, show that. 
    // If no ID or not found, fallback to first product (or handle as 404).
    const selectedProduct = products.find(p => p.id === id) || products[0];

    // If URL has an ID that doesn't exist, we might want to redirect or just show default (soft fail).
    // For now, let's just use the selectedProduct (which falls back to products[0]).
    ...
    // Helper to switch product (navigate to new URL)
      ...
      `}</style>
            ...
            <div className="container mx-auto px-4 mb-4 md:mb-6">
                <div className="text-xs md:text-sm text-gray-500 flex items-center gap-2">
                    <Link to="/" className="hover:text-rose-500">Home</Link> / <span>Shop</span> / <span className="text-gray-900 font-medium">{selectedProduct.name}</span>
                </div>
            </div>
            ...
            <div className="container mx-auto px-4">
                <div className="grid lg:grid-cols-2 gap-8 md:gap-12 mb-12 md:mb-20">
                    ...
                    <div className="flex flex-col">
                        ...
                        <h1 className="text-2xl md:text-4xl font-serif font-bold text-gray-900 mb-3 md:mb-4">{selectedProduct.name}</h1>
                ...
                {/* --- US VS THEM & ROUTINE & GUARANTEE & FAQ (Static Content) --- */}
                {/* We keep these sections as they are generic to the brand */}
